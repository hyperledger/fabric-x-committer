...
// TestRelayWithoutCheckpointFeedbackKeepsRunning covers a feedback that carries no signal:
// the per-TX statuses stay authoritative and intake is untouched. An absent feedback never
// reaches here -- the caller takes the clear-the-gate branch instead (see processStatusBatch).
