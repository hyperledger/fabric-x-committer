...
// permanent duplicate), so the batch carries a feedback and nothing else. It must still
// reach the sidecar, and the checkpoint's node must still be released: every other release
// path is keyed off a status, and a node left in txBeingValidated keeps its writes in the
// global dependency detector, so the re-delivered checkpoint would detect a write-write
// dependency on its own stale node and never commit even after the hash lands.
