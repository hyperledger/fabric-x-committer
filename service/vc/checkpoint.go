...
// checkpointTx is the single `_checkpoint` write in a batch, decoded into the fields the
// verification needs. ref locates the checkpoint TX itself: the feedback carries the full
// reference, not just the txID, because that is what lets the coordinator release the TX's
// dependency-graph node without a per-TX status.
...
// (checkSystemNamespace), so a malformed one never reaches the VC.
...
//
// An undecodable key, and a second `_checkpoint` TX in the same batch, fail the batch
// rather than being skipped: a skipped write stays in the batch and commits without ever
// being verified, the one outcome the verification exists to prevent. Rejecting the TX
// instead would be wrong for the second case -- there is no basis for deciding which
// attestation is the real one. Neither is reachable today (checkSystemNamespace guarantees
// the key's shape, and nothing routes two checkpoints into one batch), so both are
// broken invariants rather than bad input.
