...
func (c *transactionCommitter) commitTransactions(
	...
	// Hashing that record is not this path's concern: runSnapshotHashScheduler owns
	// every hash start, whether the snapshot is fresh, resubmitted, or orphaned by a
	// dead worker, so this path only has to make the record durable.
