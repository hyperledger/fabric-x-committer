...
realpath_cmd    ?= $(if $(shell command -v grealpath 2> /dev/null),grealpath,realpath)
go_cmd          ?= go
version         := latest
project_dir     := $(shell dirname $(shell ${realpath_cmd} $(firstword $(MAKEFILE_LIST))))
...
lint-proto: FORCE $(PROTOS_SENTINEL)
	...
	@api-linter \
		...
		$(shell find ${project_dir}/api -name '*.proto' -exec ${realpath_cmd} --relative-to ${project_dir}/api {} \;)
