...
func CreateLoadGenNamespacesTX(policy *PolicyProfile) (*protoloadgen.TX, error) {
	factory, factoryErr := NewTxBuilderFactory(policy, nil)
	if factoryErr != nil {
		return nil, factoryErr
	}
	_, ok := factory.TxSigner.HashSigners[types.MetaNamespaceID]
	if !ok {
		return nil, errors.New("no meta namespace signer found; cannot create namespaces")
	}

	tx, err := CreateNamespacesTX(policy, 0)
	if err != nil {
		return nil, err
	}
	return factory.MakeTx(tx)
}
