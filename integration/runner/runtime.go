...
type (
	...
	CommitterRuntime struct {
		...
		TxBuilder *workload.TxBuilderFactory
...
func (c *CommitterRuntime) AddOrUpdateNamespaces(t *testing.T, namespaces ...string) {
	...
	var err error
	c.TxBuilder, err = workload.NewTxBuilderFactory(c.SystemConfig.Policy, nil)
	require.NoError(t, err)
}
...
// MakeTx creates an enveloped TX from a committer's TX.
func (c *CommitterRuntime) MakeTx(t *testing.T, tx *protoblocktx.Tx) *protoloadgen.TX {
	t.Helper()
	lgTx, err := c.TxBuilder.MakeTx(tx)
	require.NoError(t, err)
	return lgTx
}
...
func (c *CommitterRuntime) MakeAndSendTransactionsToOrderer(
	...
	for i, namespaces := range txsNs {
		...
		var err error
		txs[i], err = c.TxBuilder.MakeTx(tx)
		require.NoError(t, err)
	}
